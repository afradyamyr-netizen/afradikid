...
<ApiApprovalNotifier T={T} onNavigateToSecurity={()=>{ setATab('security'); try{ window.scrollTo({top:0, behavior:'smooth'}); }catch{} }} />
 ...
 const Box=useMemo(()=>({title,children}:any)=><section className="zkad-panel-card" style={{marginBottom:12}}><h3 style={{fontSize:13.5,color:T.ttl,margin:'0 0 12px',fontWeight:800,lineHeight:1.6,display:'flex',alignItems:'center',gap:7}}>{title}</h3>{children}</section>,[T.ttl]);
  const changeConsultStatus=useCallback((id:any,status:string)=>{setSubs((list:any[])=>list.map(x=>x.id===id?{...x,consultationStatus:status,category:status,changeHistory:logChange(x,`تغییر وضعیت مشاوره به ${status}`)}:x)); void updateSubmission(id,{consultationStatus:status}).catch((e:any)=>{ console.warn('ذخیره وضعیت مشاوره ناموفق بود', e); }); void updateSubmission(id,{consultationStatus:status}).catch((e:any)=>{ console.warn('ذخیره وضعیت مشاوره ناموفق بود', e); }); if(String(status)==='مشاوره شده'){const cur=(subs||[]).find((x:any)=>String(x.id)===String(id)); if(cur)autoGeneratePlansIfEmpty(id,cur.mealPlan,(d:{mealPlan:string;sportPlan:string})=>setSubs((prev:any[])=>prev.map((x:any)=>x.id===id?{...x,...(d.mealPlan?{mealPlan:d.mealPlan,showMealPlan:true}:{}),...(d.sportPlan?{sportPlan:d.sportPlan,showSportPlan:true}:{}),plansAiAt:Date.now()}:x)));}},[subs,setSubs]);
