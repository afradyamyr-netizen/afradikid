    ...
    <Box title={title}>
      ...
      {items.map((it: any, i: number) => (
        <details key={it.id || i} style={{ border: `1px solid ${T.brd}`, borderRadius: 12, padding: 10, marginBottom: 8, background: T.badge }}>
          ...
          <p style={{ fontSize: 10.5, color: T.mut, margin: '0 0 6px', lineHeight: 1.7 }}>هاف لایت‌های رنگی می‌توانند هر جای متن/توضیحات قرار بگیرند — فیلد عددی کنار رنگ = «بعد از چند پاراگراف بیاید» (۰ یعنی مجموعهٔ ثابت بالای متن). رنگ پاستیلی انتخاب کنید تا متن خوانا بماند.</p>
          {(() => {
