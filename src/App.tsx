...
function App(){
 ...
 // تجزیه زودهنگام روی mount (بدون وابستگی به cfg) برای رفع باگ «اولین کلیک کار نمی‌کند»
 // نکته: به‌جای history.replaceState از navigate استفاده می‌کنیم تا React Router مسیر را به‌روز کند
 useEffect(()=>{
   ...
   if (parsed) {
     ...
     try {
       ...
       if (path && path !== '/' && new RegExp(`/${parsed.raw}/?$`, 'i').test(path)) {
         // اگر لینک گسترش‌یافته بود، نرمال نکن؛ بگذار effect های زیر مسیر را تعیین کنند
