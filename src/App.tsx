...
function App(){
 ...
 // نرمال‌سازی URL بعد از تطبیق موفق (مستقل از mount زودهنگام)
 useEffect(()=>{
 ...
 // لینک ارجاع گسترش‌یافته: باز کردن خودکار تب/دوره یا شروع مستقیم ثبت‌نام
 useEffect(()=>{
   const rt = referralTarget;
   if (!rt) return;
   const tabs = (cfg as any).courseTabs || [];
   if (rt.tabCode) {
     ...
     if (tab) {
       ...
       if (typeof rt.courseIndex === 'number') {
         ...
         if (target) {
           // شروع مستقیم روند ثبت‌نام دوره
           setShipModal(target);
         }
       } else {
         // رفتن به صفحه معرفی دوره‌ها و باز کردن آن تب
         if (view !== 'courses') setView('courses');
       }
     }
   }
   // فقط یک‌بار با اولین تطبیق معتبر اجرا شود
   // eslint-disable-next-line react-hooks/exhaustive-deps
 }, [referralTarget?.raw]);
