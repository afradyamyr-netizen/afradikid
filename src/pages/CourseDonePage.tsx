...
export default function CourseDonePage(){
 ...
 const [message]=useState(()=>{try{const used=JSON.parse(sessionStorage.getItem('zkid_used_course_msgs')||'[]');const r=getRandomMessageTracked(successSentences,used);sessionStorage.setItem('zkid_used_course_msgs',JSON.stringify(r.newUsedIndices));return r.message}catch{return successSentences[Math.floor(Math.random()*successSentences.length)] || 'به جمع خانواده فرزند من خوش آمدید'}});
 return <div style={S.page}><Helmet><title>ثبت‌نام دوره | فرزند من</title><meta name="description" content="تکمیل ثبت‌نام دوره تخصصی رشد و تغذیه کودکان و نوجوانان" /><meta name="robots" content="noindex, follow" /></Helmet><style>{css}</style><div style={{...S.card,maxWidth:460,textAlign:'center',padding:'20px 18px'}}>
