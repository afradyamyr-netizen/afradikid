...
export default function CourseDetailView({ course, T, lang, onClose, onRegister, onConsult, countries, educationalMedia = [], parentExperienceMedia = [], mediaVpnOn = false, hasReferral = false, referralConsultant = null, onOpenCourse, consultFocusSignal = 0 }: Props) {
  ...
  // SEO صفحه دوره (قابل ایندکس در گوگل): title/meta/canonical/OG + JSON-LD Course
  const seoTitle = `${title} | ${isFa ? (cfg?.siteTitle || 'سامانه') : (cfg?.siteTitle || 'Platform')}`;
  ...
  const courseLd = JSON.stringify({
    ...
    ...(seoImg ? { image: [seoImg] } : {}),
    provider: { '@type': 'Organization', name: isFa ? (cfg?.siteTitle || '') : (cfg?.siteTitle || '') },
